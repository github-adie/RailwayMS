...
def cancel_tickets(train_number, seat_number):
    ...
    if train_data:
        ...
        conn.commit()
        st.success(f"Successfully canceled seat {seat_number} from train {train_number}.")
    else:
        st.error(f"No such Train with Number {train_number} is available")
        ...
        if st.button("Search by Train Number"):
            if train_number:
                ...
                if train_data:
                    ...
                    st.table(pd.DataFrame([train_data], columns=["Train Number", "Train Name", "Departure Date", "Starting Destination", "Ending Destination"]))
                else:
